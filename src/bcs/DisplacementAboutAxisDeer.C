...
#include "DisplacementAboutAxisDeer.h"
#include "Function.h"

registerMooseObject("DeerApp", DisplacementAboutAxisDeer);

InputParameters DisplacementAboutAxisDeer::validParams() {
  InputParameters params = DirichletBCBase::validParams();
  params.addClassDescription(
      "Implements a boundary condition that enforces rotational"
      "displacement around an axis on a boundary. To remove when PR #15390 get "
      "merged");
  addDisplacementAboutAxisDeerParams(params);
...
void addDisplacementAboutAxisDeerParams(InputParameters &params) {
  MooseEnum units("degrees radians");
  params.addRequiredParam<FunctionName>(
      "function", "The function providing the total angle of rotation or the "
                  "angular velocity.");
  params.addRequiredParam<MooseEnum>(
      "angle_units", units,
      "The units of the angle of rotation. Choices are:" + units.getRawNames());
...
DisplacementAboutAxisDeer::DisplacementAboutAxisDeer(
    const InputParameters &parameters)
    : DirichletBCBase(parameters), _component(getParam<int>("component")),
      ...
      _ndisp(coupledComponents("displacements")), _disp_old(_ndisp),
...
void DisplacementAboutAxisDeer::initialSetup() {
...
Real DisplacementAboutAxisDeer::computeQpValue() {
...
ColumnMajorMatrix
DisplacementAboutAxisDeer::rotateAroundAxis(const ColumnMajorMatrix &p0,
                                            const Real angle) {
...
void DisplacementAboutAxisDeer::calculateUnitDirectionVector() {
  Real magnitude = _axis_direction.norm();
  _axis_direction /= magnitude;
}

void DisplacementAboutAxisDeer::calculateTransformationMatrices() {
  // These parts of the transformation matrix only depend on the axis of
  // rotation:

  Real length = _axis_direction.norm_sq();
  Real v = _axis_direction(1) * _axis_direction(1) +
           _axis_direction(2) * _axis_direction(2);
