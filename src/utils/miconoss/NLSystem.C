...
vecD
NLSystem::assembleR(const vecD & lm) const
{
...
matrixD
NLSystem::assembleJ(const vecD & lm) const
{
...
void
NLSystem::updateEquationConstants()
{
...
matrixD
NLSystem::unscaleJacobian(const matrixD & Jscaled)
{
  matrixD J = Jscaled;
  // scale equations
  for (uint i = 0; i < _nx; i++)
...
matrixD
NLSystem::getDResidualDParams(const std::vector<std::string> & pname)
{
  ...
  for (uint i = 0; i < _nx; i++)
  {
    ...
    for (uint p = 0; p < n_param; p++)
      dresdp[p][i] = _eq[i]->getDEquationDParam(pname[p]) * _sys_vars->getDVarDVarScaled(i);
  }
...
matrixD
NLSystem::getDSystemVarsDParams(const std::vector<std::string> & pname, const vecD & lm)
{
  updateEquationConstants();
...
void
NLSystem::updateEquationScaling() const
{
  for (uint i = 0; i < _nx; i++)
