...
NLPreEquationEvalautionCalc::NLPreEquationEvalautionCalc(
    NLSystemVars *const sysvars, const NLSystemParameters *sysparams,
    const std::vector<std::string> &value_names)
    : _sys_vars(sysvars), _sysparams(sysparams), _n_values(value_names.size()),
      _n_vars(_sys_vars->getNVars()), _values(_n_values),
      _dvalues_dx(_n_values, std::vector<double>(_n_vars)),
      _values_explicit(_n_values), _dvalues_dx_explicit(_n_vars, 0) {

  for (uint i = 0; i < _n_values; i++)
    _name_index_map.insert(std::pair<std::string, uint>(value_names[i], i));
}

void NLPreEquationEvalautionCalc::updateAll(const bool updade_explicit) {
  if (updade_explicit)
...
uint NLPreEquationEvalautionCalc::getValueIndex(
...
double NLPreEquationEvalautionCalc::getValue(const std::string &vname,
...
vecD NLPreEquationEvalautionCalc::getDValueDX(const std::string &vname,
                                              const bool implicit) const {
  if (implicit)
...
void NLPreEquationEvalautionCalc::setValue(const std::string &vname,
                                           double value, const bool implicit) {
  if (implicit)
...
void NLPreEquationEvalautionCalc::setDValueDX(const std::string &vname,
                                              const vecD &dvaluedx,
                                              const bool implicit) {
  if (implicit)
