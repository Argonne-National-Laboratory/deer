...
InputParameters
EffectiveStressInterfaceMaterial::validParams()
{
...
void
EffectiveStressInterfaceMaterial::computeQpProperties()
{

  Real eff_stress_master = EffectiveStressTools::getQuantity(
      _stress_master[_qp], _effective_stress_type, _params_vector);

  Real eff_stress_slave =
      EffectiveStressTools::getQuantity(_stress_slave[_qp], _effective_stress_type, _params_vector);

  _effective_stress[_qp] =
      InterfaceValueTools::getQuantity(_interface_value_type, eff_stress_master, eff_stress_slave);
}

void
EffectiveStressInterfaceMaterial::initQpStatefulProperties()
{
  if (_stateful)
