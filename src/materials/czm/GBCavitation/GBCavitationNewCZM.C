...
#include "GBCavitationNewCZM.h"
#include "NLSystem.h"
registerMooseObject("DeerApp", GBCavitationNewCZM);

InputParameters GBCavitationNewCZM::validParams() {
  ...
  if (_element_failed_old[_qp] != 0) {
    ...
    for (uint i = 0; i < 3; i++)
      mooseAssert(
          ...
          "GBCavitationNewCZM _interface_traction[_qp] " + std::to_string(i) +
              " is not finite: " + std::to_string(_interface_traction[_qp](i)));

    mooseAssert(std::isfinite(_a[_qp]),
                "GBCavitationNewCZM _a[_qp] is not finite: " +
                    std::to_string(_a[_qp]));
...
void GBCavitationNewCZM::tractionDecay() {
  Real P = 1.;
  Real dP_djump = 0.;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i == 0)
      C *= P;
    _interface_traction[_qp](i) = (_interface_displacement_jump[_qp](i) -
      ...
      if (i == j) {
        ...
        if (i == 0) {
          _dinterface_traction_djump[_qp](i, j) +=
              (_interface_displacement_jump[_qp](i) -
               _jump_at_failure[_qp](i) * decay_factor) *
              C / P * dP_djump;
        }
      } else
...
void GBCavitationNewCZM::initQpStatefulProperties() {
...
void GBCavitationNewCZM::computeAverageBulkPorperties() {
...
void GBCavitationNewCZM::initNLSystemParamter(
    std::vector<std::string> &pname, vecD &pvalue,
    std::vector<std::string> &rate_pname, vecD &rate_pvalue) {
  pname = {"dt",
           "dt_accum",
           "max_ab",
...
void GBCavitationNewCZM::getInitPropertyValuesFromParams(
...
void GBCavitationNewCZM::getInitPropertyValuesFromUO(
    Real &FN_NI, Real &Nmax_NI, Real &a0, Real &b0, Real &psi, Real &D_GB,
    Real &E_GB, Real &G_GB, Real &eta_sliding, Real &sigma_0, Real &thickness,
    Real &beta_exponent, Real &n_exponent) const {
