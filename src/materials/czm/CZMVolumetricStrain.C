...
InputParameters
CZMVolumetricStrain::validParams()
{
...
CZMVolumetricStrain::CZMVolumetricStrain(const InputParameters & parameters)
  : InterfaceMaterial(parameters),
    ...
    _czm_total_rotation(
        getMaterialPropertyByName<RankTwoTensor>(_base_name + "czm_total_rotation")),
    _czm_total_strain(declarePropertyByName<RankTwoTensor>(_base_name + "czm_total_strain")),
    _czm_normal_strain(declarePropertyByName<RankTwoTensor>(_base_name + "czm_normal_strain")),
    _czm_sliding_strain(declarePropertyByName<RankTwoTensor>(_base_name + "czm_sliding_strain")),
...
void
CZMVolumetricStrain::initQpStatefulProperties()
{
...
void
CZMVolumetricStrain::computeQpProperties()
{
...
void
CZMVolumetricStrain::computeInterfaceStrain()
{
  const RealVectorValue n_average = _czm_total_rotation[_qp] * RealVectorValue(1.0, 0.0, 0.0);
  const RankTwoTensor u_outer_n = outer_product(_displacement_jump_global[_qp], n_average);
  const RankTwoTensor n_outer_n = outer_product(n_average, n_average);

  _czm_total_strain[_qp] = (u_outer_n + u_outer_n.transpose()) / 2. * _dadA_mp[_qp];
  _czm_normal_strain[_qp] = _displacement_jump_global[_qp] * n_average *
                            (n_outer_n + n_outer_n.transpose()) / 2. * _dadA_mp[_qp];
