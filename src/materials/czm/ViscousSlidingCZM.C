...
ViscousSlidingCZM::ViscousSlidingCZM(const InputParameters &parameters)
    : PureElasticCZM(parameters),
      ...
      _interface_traction_old(getMaterialPropertyOldByName<RealVectorValue>(
          _base_name + "interface_traction")),
      _interface_displacement_jump_old(
          getMaterialPropertyOldByName<RealVectorValue>(
              _base_name + "interface_displacement_jump")),
      _interface_displacement_jump_dot(declarePropertyByName<RealVectorValue>(
          _base_name + "interface_displacement_jump_dot")) {}
...
void ViscousSlidingCZM::ComputeShearTractionDerivatives(
  ...
  for (unsigned int i = 1; i < 3; i++) {
    ...
    Real dTsi_dC = -_dt / eta * std::exp(-_dt * C / eta) *
                   (_interface_traction_old[_qp](i) -
                    eta * _interface_displacement_jump_dot[_qp](i));
    Real dTsi_deta =
