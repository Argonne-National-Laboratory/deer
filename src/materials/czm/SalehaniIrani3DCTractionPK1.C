...
#include "SalehaniIrani3DCTractionPK1.h"

registerMooseObject("DeerApp", SalehaniIrani3DCTractionPK1);

InputParameters SalehaniIrani3DCTractionPK1::validParams() {
...
SalehaniIrani3DCTractionPK1::SalehaniIrani3DCTractionPK1(const InputParameters & parameters)
...
void
SalehaniIrani3DCTractionPK1::computeTractionIncrementAndDerivatives()
{
...
RealVectorValue
SalehaniIrani3DCTractionPK1::computeTraction()
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    traction_local_inc(i) = _max_allowable_traction[i] * lambda / _delta_u0[i] * A *
...
RankTwoTensor
SalehaniIrani3DCTractionPK1::computeTractionDerivatives()
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    {
      Real alpha = (j == 0) ? 1. : 2.;
      Real dA_djump = A * (-alpha / _delta_u0[j] *
                           std::pow(_displacement_jump[_qp](j) / _delta_u0[j], alpha - 1.));
      Real dB_djump =
          -alpha / _delta_u0[j] * std::pow(_displacement_jump[_qp](j) / _delta_u0[j], alpha - 1.);
      ...
      dtraction_djumpinc(i, j) =
          dA_djump * (_displacement_jump_inc[_qp](i) + _displacement_jump[_qp](i) * B) +
