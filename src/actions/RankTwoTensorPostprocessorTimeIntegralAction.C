...
registerMooseAction("DeerApp", RankTwoTensorPostprocessorTimeIntegralAction, "add_postprocessor");

InputParameters
RankTwoTensorPostprocessorTimeIntegralAction::validParams()
{
  ...
  params.addRequiredParam<std::vector<PostprocessorName>>("pp_base_names",
                                                          "a vector of postprocessor base names.");
...
RankTwoTensorPostprocessorTimeIntegralAction::RankTwoTensorPostprocessorTimeIntegralAction(
...
void
RankTwoTensorPostprocessorTimeIntegralAction::act()
{
    ...
    for (unsigned int pp = 0; pp < _pp_base_names.size(); pp++)
      for (auto entry : _tensor_map)
      {
        ...
        _problem->addPostprocessor(
            "TimeIntegralPostprocessor", _base_out_name[pp] + "_" + entry.second, params_pp);
      }
  }
}
