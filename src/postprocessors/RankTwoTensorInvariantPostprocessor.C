...
void RankTwoTensorInvariantPostprocessor::initialSetup() {
  ...
  for (auto entry : tensor_map) {
    ...
    _pps_values[i][j] = (&getPostprocessorValueByName(
        _rank_two_tensor_base_name + "_" + entry.second));
    if (i != j)
      _pps_values[j][i] = (&getPostprocessorValueByName(
          _rank_two_tensor_base_name + "_" + entry.second));
  }
}
